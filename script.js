...
function handleTerminalRouter(input, displayContainer) {
  ...
  if (sessionState === "ENDLOOP") {
    if (cleanInput === "1") {
      ...
      terminateTerminal(displayContainer, inputLine);
    } else {
...
if (cleanInput.toLowerCase().startsWith("/read ")) {
  ...
  executeReadCommand(bookQuery, displayContainer, inputLine);
  return;
}
...
// The Loop is a ritual ofc

function executeRitualLoop(text, container, inputLine) {
  isProcessing = true;
  ...
  const logLinesContainer = container.querySelector(".log-lines");
...
function renderEndingChoices(container) {
  container.innerHTML = `
...
function terminateTerminal(displayContainer, inputLine) {
  ...
  displayContainer.innerHTML = `
        <div class="termination-msg">
            <div id="termination-output"></div>
        </div>
    `;
